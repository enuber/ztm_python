...
class Employee:
    ...
    def calculate_salary(self, salary, hours_worked):
        overtime = 0
        max_weekly_hours = 40
        if hours_worked > max_weekly_hours:
            overtime = hours_worked - max_weekly_hours
        self.salary = self.salary + (overtime * (salary//max_weekly_hours))

    def assign_department(self, new_department):
        self.department = new_department

    def print_employee_details(self):
        print(f'{self.name} has an id of {self.id}. {self.name} is in the {
              self.department} department and makes {self.salary}')
