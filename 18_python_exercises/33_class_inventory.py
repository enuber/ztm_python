...
class Inventory:
    ...
    def update_item(self, item_id, stock_count, price):
        if item_id in self.inventory.keys():
            ...
            print(f'Item with ID number of {
                  item_id} was not found in the inventory')

    def check_item_details(self, item_id):
        if item_id in self.inventory.keys():
            item = self.inventory[item_id]
            return f"Product Name: {item['item_name']}, Stock Count: {item['stock_count']}, Price ${item['price']:.2f}"
        else:
            print(f'Item with ID number of {
                  item_id} was not found in the inventory')
